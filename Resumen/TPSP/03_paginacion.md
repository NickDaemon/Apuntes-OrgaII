...
Idirección lógicae. El resto del mapa muestra el rango para la pila del kernel, desde
...
a)  ¿Cuántos niveles de privilegio podemos definir en las estructuras de
    paginación?
- **Podemos tener 4 niveles de privilegio, nivel 0, 1, 2 y 3.**
- **Pero en la práctica solo se usan dos, nivel 0 (kernel o supervisor) y nivel 3 (usuario).**
...
1. ***Dirección Lógica a Lineal/virtual:***
- **La dirección lógica esta compuesta por un selector de segmento y un offset.**
- **La CPU , traduce esta dirección lógica en una virtual usando la GDT.**
- **Usa el selector como indice en la GDT y accede al decriptor correspondiente.**
- **De dicho decriptor obtiene la base del segmento al que quiere acceder y suma esa base con el offset de la dirección lógica obteniendo una dirección virtual.**

2. ***Dirección Virtual a Física:***
- **La MMU toma esta dirección virtual y se encarga de traducirla a una dirección física.Osea se encarga de encontrar en la memoria física, donde esta el recurso al cual la CPU quiere acceder.**
- **Con el registro CR3 obtiene la dirección física del directorio (PD) correspondiente en donde se encuentra dicho recurso.**
- **Usa los 10 bits mas altos de la dirección virtual como índice en dicho directorio y obtiene la dirección física de la tabla de páginas (PT) en donde esta la página que tiene el recurso.**
- **Con los 10 bits del medio de la dirección virtual obtiene la dirección física de la página que contiene el recurso que busca.**
- **Por último usa los 12 bits mas bajos de la dirección virtual para obtener exactamente la dirección física del recurso que necesita la CPU.**


c)  ¿Cuál es el efecto de los siguientes atributos en las entradas de la
    tabla de página?

  - `D`:**Indica si la página fue modificada o no.**
  - `A`:**Indica si la página fue leida o escrita.**
  - `PCD`:**Controla si la página puede almacenarse en la caché.**
  - `PWT`:**Controla como se escribe en la caché.**
  - `U/S`:**Define el privilegio necesario para acceder a la página, si el bit esta apagado solo accede el kernel, de lo contrario pueden acceder kernel/usuario.**
  - `R/W`:**Indica los permisos de escritura, si el bit esta apagado es solo lectura de lo contrario es lectura/escritura.**
  - `P`: **Indica si la página está presente en memoria.**
...
- ***Si ambos tienen privilegio user pero difieren en el acceso , se setea con el acceso mas restrictivo, solo lectura y el privilegio queda en user.***
- ***Si diferen en el privilegio , se setea con el privilegio mas restrictivo que es supervisor/kernel, y el acceso se vuelve lectura/escritura.***
...
- **Para el directorio (PD) hace falta pedir solo una.**
- **Para las tablas de páginas , como cada una tiene 1024 entradas , con una alcanza.**
- **Para la memoria de la tarea vamos a ocupar 3, dos de código y una para la pila.**
- ***Preguntar bien sobre el kernel.***
...
- **El buffer auxiliar TLB es una caché pequeña de la MMU que almacena los bits más significativos de las direcciones virtuales y físicas (los 20 bits más altos, que representan la base de la página), junto con algunos bits de permisos.Esto permite acelerar la traducción de direcciones, ya que no es necesario recorrer constantemente el Page Directory y la Page Table; el CPU puede acceder directamente a la dirección física consultando el TLB.**
- **Si por ejemplo guardo en la tlb que la dirección A virtual se encuentra en dirección B física, y luego reasigno A a una dirección C, al consultar con la tlb encuentra que A tiene traducción pero tiene la traducción desactualizada , asigna A a B cuando en realidad ahora A se encuentra en C.Por lo tanto al introducir modificaciones es importante hacer un tlbflush para evitar usar traducciones incorrectas.**
- **Solo guarda traducciones virtuales-físicas , no guarda el contenido de las páginas por lo tanto si modificamos una entrada en la tlb , la homóloga en la tabla original no se ve afectada.**
...
b)  Completen el código de `copy_page`, ¿por qué es necesario mapear y
    ...
    CR3 con rcr3()?
- **Porque las direcciones dst_addr y src_addr son direcciones físicas, no virtuales. El procesador solo puede acceder directamente a memoria virtual, no a memoria física.**
- **Mapeamos temporalmente la página fuente (src_addr) y la de destino (dst_addr) a direcciones virtuales conocidas (como SRC_VIRT_PAGE y DST_VIRT_PAGE) y una vez mapeadas, podemos copiarlas byte a byte en modo protegido.**
- **Luego como SRC_VIRT_PAGE y DST_VIRT_PAGE son direcciones reutilizables, si no las desmapeamos, la próxima vez que se quiera copiar otra página, esas direcciones seguirían mapeadas a las páginas anteriores.**
- **SRC_VIRT_PAGE y DST_VIRT_PAGE son direcciones virtuales fijas que usamos para que el procesador pueda acceder a las direcciones físicas que queremos copiar, de esta manera sabemos que en SRC_VIRT_PAGE esta la página que queremos copiar, y DST_VIRT_PAGE sera el destino a donde copiar.**
- **El registro CR3 contiene la dirección física del directorio de páginas actual.Cuando llamamos a mmu_map_page() o mmu_unmap_page(), necesitamos saber en qué espacio de direcciones virtuales estamos mapeando las páginas.**
