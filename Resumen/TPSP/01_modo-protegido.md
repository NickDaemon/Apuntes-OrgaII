...
1. Explorando el manual Intel *Volumen 3: System Programming. Sección 2.2 Modes of Operation*. ¿A qué nos referimos con modo real y con modo protegido en un procesador Intel? ¿Qué particularidades tiene cada modo?
    - **Modo Real:**
        - **Por retrocompatibilidad, es el modo en el que arrancan todos los procesadores intel.**
        - **Ejecuta instrucciones de 16 bits y permite direccionar hasta 1 MiB de memoria.**
        - **En este modo el procesador tiene acceso total al hardware y no hay protección de memoria.**
    - **Modo Protegido:**
        - **Es el modo "nativo" de todos los procesadores modernos Intel.**
        - **Ejecuta instrucciones de 32 bits y permite direccionar hasta 4 GiB de memoria.**
        - **Permite niveles de protección y control de acceso a memoria y recursos.**
...
2. Comenten en su equipo, ¿Por qué debemos hacer el pasaje de modo real a modo protegido? ¿No podríamos simplemente tener un sistema operativo en modo real? ¿Qué desventajas tendría?
    - **Por retrocompatibilidad , todos los procesadores deben arrancar en modo real, pero en este modo no podemos hacer uso de todos los recursos que nos brindan los procesadores modernos.Por esto, los sistemas operativos modernos arrancan en modo real solo para inicializar el hardware y luego “saltan” a modo protegido para poder usar todas las capacidades del CPU y garantizar estabilidad y seguridad.**
...
3. Busquen el manual *volumen 3 de Intel en la sección 3.4.5 Segment Descriptors*. ¿Qué es la GDT? ¿Cómo es el formato de un descriptor de segmento, bit a bit? Expliquen brevemente para qué sirven los campos *Limit*, *Base*, *G*, *P*, *DPL*, *S*. También pueden referirse a los slides de la clase teórica, aunque recomendamos que se acostumbren a consultar el manual.
    - **Es un arreglo de descriptores que indica cómo está segmentada la memoria en modo protegido, qué permisos tiene cada segmento y quién puede acceder a ellos.**
    - **Cada descriptor define un segmento de memoria, su tamaño, base y nivel de protección.**
        1. **Limit: Nos dice el tamaño del segmento. El procesador interpreta este límite en dos maneras:**
            - **Si G esta apagado, el tamaño del segmento tiene rango entre 1 Byte a 1 MiB, con incrementos de a Bytes.**
            - **Si G esta prendido, el tamaño del segmento tiene rango entre 4 KiB a 4 GBytes, con incrementos de a 4 KiB.**
        2. **Base: Nos dice en donde comienza el segmento.**
        3. **G: Define como se interpreta el limite del segmento, si esta apagado se mide en Bytes y si esta prendido en bloques de 4KiB.**
        4. **P: Indica si el segmento esta presente en memoria o no.**
        5. **DPL: Define el nivel de "privilegio" del segmento. Tiene un rango entre 0 y 3 con 0 el nivel de mayor privilegio, y se usa para controlar el acceso al segmento.**
        6. **S: Si este bit esta apagado , nos dice que el segmento es de sistema , si esta prendido nos dice que el segmento puede ser de código o datos.**
...
6. En el archivo `gdt.h` observen las estructuras: `struct gdt_descriptor_t` y el `struct gdt_entry_t`. ¿Qué creen que contiene la variable `extern gdt_entry_t gdt;` y `extern gdt_descriptor_t GDT_DESC;`?
    - **extern gdt_entry_t gdt es la GDT. Cada elemento del array representa un decriptor de segmento.**
    - **extern gdt_descriptor_t GDT_DESC toma el rol del GDTR. Con este registro el procesador sabe donde empieza la GDT y cual es su tamaño.**
...
10. Busquen qué hace la instrucción LGDT en el Volumen 2 del manual de Intel. Expliquen con sus palabras para qué sirve esta instrucción. En el código, ¿qué estructura indica donde está almacenada la dirección desde la cual se carga la GDT y su tamaño? ¿dónde se inicializa en el código?
    - **Esta instrucción carga el valor que le damos como argumento en la GDTR, para que luego el procesador sepa desde donde cargar la GDT.**
    - **La estructura que indica donde esta almacenada la dirección de la GDT es GDT_DESC**
    - **Se inicializa en gdt.h.**
