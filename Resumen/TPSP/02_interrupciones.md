...
- **offset (parte baja y alta): Es la dirección del handler correspondiente a esa interrupción.**
- **segsel: Es el selector de segmento de código que se usará al saltar al handler.**
- **type: Tipo de puerta (interrupt gate, trap gate).**
- **dpl: Indica el nivel mínimo de privilegio que puede invocar esta interrupción con int n.**
- **p: Indica que la entrada de la IDT está presente y activa.**
...
- **Como las rutinas de interrupción del kernel corren en nivel 0, y el código del kernel está en el segmento de código del kernel, el selector correcto es GDT_CODE_0_SEL.**
...
- **El prólogo salva el estado del proceso interrumpido para que, al volver, todo quede igual.**
- **Al final del handler, el epílogo restaura el contexto.**
- **ret simplemente saca una dirección de retorno (EIP) de la pila y vuelve a esa instrucción.En cambio iret restaura todo el contexto que guardó el procesador al entrar a la interrupción.Esto permite volver exactamente al contexto previo, incluso si se cambió de nivel (por ejemplo, de nivel 0 a nivel 3).**
